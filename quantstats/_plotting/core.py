...
def plot_histogram(
    ...
    if benchmark is not None:
        if isinstance(returns, _pd.Series):
            combined_returns = benchmark.to_frame().join(returns.to_frame()).stack().reset_index().rename(columns={"level_1": "", 0: "Returns"})
        elif isinstance(returns, _pd.DataFrame):
            combined_returns = benchmark.to_frame().join(returns).stack().reset_index().rename(columns={"level_1": "", 0: "Returns"})
        x = _sns.histplot(
...
def plot_rolling_beta(
    ...
    if isinstance(returns, _pd.Series):
        ...
        beta = {col: _stats.rolling_greeks(returns[col], benchmark, window1)["beta"].fillna(0) for col in returns.columns}
